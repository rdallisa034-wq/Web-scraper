...
def invalidate_and_delete_token(token: str, reason: str = "") -> None:
    """Hapus token habis/kadaluarsa agar tidak bisa diisi ulang / disalahgunakan."""
    data = ensure_tokens_file()
...
def token_status(token: str) -> Tuple[str, str, Optional[Dict[str, Any]]]:
    ...
    Returns: (status, message, session_or_none)
      status: ok | missing | inactive | expired | no_credit
    """
...
def buy_token_link() -> str:
    """URL tombol beli token (WhatsApp / halaman)."""
    s = get_settings()
