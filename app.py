...
def _require_login() -> bool:
    ...
    if ok:
        ...
        if account:
            ...
            log_access(account["username"], "login", "success")
            st.rerun()
        else:
    ...
    st.divider()
    st.caption(f"Default admin: `{DEFAULT_ADMIN_USER}` / `{DEFAULT_ADMIN_PASS}` — ganti setelah login.")
    return False
...
if menu == "Admin":
    ...
    st.header("⚙️ Admin")
    tab_users, tab_logs, tab_pw = st.tabs(["Kelola user", "Log akses", "Ganti password"])
