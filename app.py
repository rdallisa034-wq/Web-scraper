...
if menu == "Scrape":
    ...
    if st.button(
        ...
        # Token: cek + potong credit / catat pakai waktu
        if user.get("auth_type") == "token":
            ...
            if not ok_c:
                ...
                st.stop()
            if mode == "credit":
                ok_d, msg_d, left = deduct_token_credits(user["token"], cost)
                if not ok_d:
                    st.session_state.err = msg_d
                    st.error(msg_d)
                    st.stop()
                log_access(user["username"], "credit_deduct", f"-{cost} sisa={left}")
            else:
                mark_token_used(user["token"])
                log_access(user["username"], "time_token_use", mdy)
            st.session_state.auth_user = refresh_token_session(user["token"]) or user
            user = st.session_state.auth_user
        if watch:
