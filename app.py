    ...
    if st.button(
        ...
        if user.get("auth_type") == "token":
            ...
            if mode == "credit":
                ...
                if left <= 0:
                    # Credit habis setelah scrape ini — beritahu, lalu logout
                    st.session_state.auth_user = None
                    _show_token_dead_screen(
                        "Credit token habis setelah scrape ini dan token telah dihapus. "
                        "Silakan beli/minta token baru ke admin."
                    )
            else:
                mark_token_used(user["token"])
                log_access(user["username"], "time_token_use", mdy)
            refreshed = refresh_token_session(user["token"])
            if not refreshed and user.get("token_mode") == "time":
                st.session_state.auth_user = None
                _show_token_dead_screen(
                    "Token waktu sudah kadaluarsa dan telah dihapus. Silakan beli/minta token baru."
                )
            if refreshed:
                st.session_state.auth_user = refreshed
                user = refreshed
