...
if menu == "Scrape":
    ...
    with st.expander("Opsi tambahan"):
        ...
        mascots = st.checkbox("Ambil mascot (lebih lambat)", value=False)
...
# ===================== EDIT =====================
    ...
    if not files:
        ...
        if path != st.session_state.editor_path:
            st.session_state.editor_text = open(path, encoding="utf-8").read()
        ...
        with c2:
            repl = st.text_input("Ganti")

        r1, r2, r3 = st.columns(3)
        with r1:
            if st.button("Replace all", use_container_width=True):
                if find:
                    t = st.session_state.editor_text
                    st.session_state.editor_text = re.compile(re.escape(find), re.I).sub(repl, t)
                    open(path, "w", encoding="utf-8").write(st.session_state.editor_text)
                    st.success("Diganti & disimpan.")
                    st.rerun()
        with r2:
            if st.button("Simpan", use_container_width=True):
                open(path, "w", encoding="utf-8").write(
                    st.session_state.get("ed_area", st.session_state.editor_text)
                )
                st.success("Disimpan.")
        with r3:
        ...
        st.text_area("Isi", value=st.session_state.editor_text, height=300, key="ed_area")
        text = st.session_state.get("ed_area", st.session_state.editor_text)
        st.caption(f"{len(text)} karakter")
        st.code(text[:12000] + ("…" if len(text) > 12000 else ""))
        st.download_button("⬇️ Download", data=text.encode(), file_name=os.path.basename(path), mime="text/plain")
