        ...
        for u in users:
            ...
            with c1:
                if u["active"] and not suspended_until:
                    if st.button("⏸ Off", key=f"off_{u['username']}", use_container_width=True):
                        ok, msg = set_user_active(u["username"], False)
                        log_access(user.get("username", "?"), "deactivate_user", u["username"])
                        st.toast(msg)
                        st.rerun()
                elif not u["active"] and not suspended_until:
                    if st.button("▶ On", key=f"on_{u['username']}", use_container_width=True):
                        ok, msg = set_user_active(u["username"], True)
                        log_access(user.get("username", "?"), "activate_user", u["username"])
                        st.toast(msg)
                        st.rerun()
            
            with c2:
                if suspended_until:
                    if st.button("✓ Pulihkan", key=f"unsuspend_{u['username']}", use_container_width=True):
                        ok, msg = unsuspend_user(u["username"])
                        log_access(user.get("username", "?"), "unsuspend_user", u["username"])
                        st.toast(msg)
                        st.rerun()
                else:
                    if st.button("⏸ Suspend", key=f"suspend_{u['username']}", use_container_width=True):
                        st.session_state[f"suspend_modal_{u['username']}"] = True
            
            with c3:
                if is_banned:
                    if st.button("✓ Buka Ban", key=f"unban_{u['username']}", use_container_width=True):
                        ok, msg = set_user_active(u["username"], True)
                        log_access(user.get("username", "?"), "unban_user", u["username"])
                        st.toast(msg)
                        st.rerun()
                else:
                    if st.button("🚫 Ban", key=f"ban_{u['username']}", use_container_width=True):
                        ok, msg = set_user_active(u["username"], False)
                        log_access(user.get("username", "?"), "ban_user", u["username"])
                        st.toast(msg)
                        st.rerun()
            ...
            with c4:
                if st.button("🗑 Hapus", key=f"del_{u['username']}", use_container_width=True):
                    ...
                    if ok:
                        log_access(user.get("username", "?"), "delete_user", u["username"])
                    st.toast(msg)
                    st.rerun()
            
            # Suspend modal
            if st.session_state.get(f"suspend_modal_{u['username']}"):
                st.info(f"Tangguhkan akun {u['username']}")
                hours = st.number_input(f"Jam (hours)##sus_{u['username']}", min_value=1, max_value=720, value=24)
                if st.button(f"✓ Tangguhkan {hours}h", key=f"do_suspend_{u['username']}", use_container_width=True):
                    ok, msg = suspend_user(u["username"], hours)
                    log_access(user.get("username", "?"), "suspend_user", f"{u['username']} untuk {hours}h")
                    st.toast(msg)
                    st.session_state.pop(f"suspend_modal_{u['username']}", None)
                    st.rerun()
