        ...
        for idx, u in enumerate(users):
            ...
            with c1:
                if u["active"] and not suspended_until:
                    if st.button("⏸ Off", key=f"off_{idx}_{u['email']}", use_container_width=True):
                        ok, msg = set_user_active(u["email"], False)
                        log_access(user.get("email", "?"), "deactivate_user", u["email"])
                        st.toast(msg)
                        st.rerun()
                elif not u["active"] and not suspended_until:
                    if st.button("▶ On", key=f"on_{idx}_{u['email']}", use_container_width=True):
                        ok, msg = set_user_active(u["email"], True)
                        log_access(user.get("email", "?"), "activate_user", u["email"])
                        st.toast(msg)
                        st.rerun()
            
            with c2:
                if suspended_until:
                    if st.button("✓ Pulihkan", key=f"unsuspend_{idx}_{u['email']}", use_container_width=True):
                        ok, msg = unsuspend_user(u["email"])
                        log_access(user.get("email", "?"), "unsuspend_user", u["email"])
                        st.toast(msg)
                        st.rerun()
                else:
                    if st.button("⏸ Suspend", key=f"suspend_{idx}_{u['email']}", use_container_width=True):
                        st.session_state[f"suspend_modal_{idx}"] = True
            
            with c3:
                if is_banned:
                    if st.button("✓ Buka Ban", key=f"unban_{idx}_{u['email']}", use_container_width=True):
                        ok, msg = set_user_active(u["email"], True)
                        log_access(user.get("email", "?"), "unban_user", u["email"])
                        st.toast(msg)
                        st.rerun()
                else:
                    if st.button("🚫 Ban", key=f"ban_{idx}_{u['email']}", use_container_width=True):
                        ok, msg = set_user_active(u["email"], False)
                        log_access(user.get("email", "?"), "ban_user", u["email"])
                        st.toast(msg)
                        st.rerun()
            
            with c4:
                if st.button("🗑 Hapus", key=f"del_{idx}_{u['email']}", use_container_width=True):
                    ok, msg = delete_user(u["email"])
                    if ok:
                        log_access(user.get("email", "?"), "delete_user", u["email"])
                    st.toast(msg)
                    st.rerun()
            
            # Suspend modal
            if st.session_state.get(f"suspend_modal_{idx}"):
                st.info(f"Tangguhkan akun {u['email']}")
                ...
                if st.button(f"✓ Tangguhkan {hours}h", key=f"do_suspend_{idx}", use_container_width=True):
                    ok, msg = suspend_user(u["email"], hours)
                    log_access(user.get("email", "?"), "suspend_user", f"{u['email']} untuk {hours}h")
                    st.toast(msg)
