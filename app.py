...
def _require_login() -> bool:
    ...
    if st.session_state.auth_user:
        # refresh credit token tiap rerun
        u = st.session_state.auth_user
        if u.get("auth_type") == "token" and u.get("token"):
            refreshed = refresh_token_session(u["token"])
            if not refreshed:
                st.session_state.auth_user = None
                st.warning("Token kadaluarsa / nonaktif / credit habis. Login lagi.")
                st.rerun()
            else:
...
with st.sidebar:
    ...
    if user.get("auth_type") == "token":
        st.caption(f"Token · 💳 {user.get('credits', 0)} credit")
        if user.get("expires_at"):
            st.caption(f"Valid s/d {user.get('expires_at')}")
    else:
...
if menu == "Admin":
    ...
    st.header("⚙️ Admin")
    tab_users, tab_tokens, tab_logs, tab_pw = st.tabs(
        ["Kelola user", "Token & credit", "Log akses", "Ganti password"]
    )
    ...
    with tab_tokens:
        ...
        st.caption(
            f"Biaya scrape: **{COST_PER_STATE}** credit / 1 state · "
            f"**{COST_ALL_STATES}** all state · **{COST_TOP25}** bila Top 25/ranking."
        )
        with st.form("create_token_form"):
            t_label = st.text_input("Label (opsional)", placeholder="client-A")
            t_credits = st.number_input("Credit", min_value=0, value=20, step=1)
            t_hours = st.number_input(
                "Berlaku (jam)", min_value=0.5, value=24.0, step=1.0,
                help="Contoh: 24 = 1 hari, 168 = 7 hari",
            )
            t_go = st.form_submit_button("Generate token", type="primary")
        if t_go:
            ok, msg, info = create_token(
                int(t_credits), float(t_hours), label=t_label, created_by=user["username"]
            )
            if ok and info:
                log_access(user["username"], "create_token", info["token"])
                st.success(msg)
                st.code(info["token"], language=None)
                st.caption(
                    f"Credit: {info['credits']} · kadaluarsa: {info['expires_at']}"
                )
            else:
                st.error(msg)
        ...
        for t in tokens:
            st.markdown(
                ...
                f"Status: **{t['status']}** · 💳 {t.get('credits',0)} / {t.get('credits_initial',0)} · "
                f"pakai {t.get('use_count',0)}x  \n"
                f"<small>dibuat {t.get('created_at','')} · valid s/d {t.get('expires_at','')}</small>",
                unsafe_allow_html=True,
            )
            a1, a2, a3, a4 = st.columns(4)
            with a1:
                if t.get("active") and t["status"] != "kadaluarsa":
            ...
            with a4:
                if st.button("+ Credit", key=f"tadd_{t['token']}", use_container_width=True):
...
if menu == "Scrape":
    ...
    if user.get("auth_type") == "token":
        st.info(
            ...
            f"(sisa kamu: **{user.get('credits', 0)}**). "
            f"Tarif: {COST_PER_STATE}/state · {COST_ALL_STATES}/all · {COST_TOP25}/Top25."
        )
    ...
    if st.button(
        ...
        # Potong credit SEBELUM scrape (token only)
        if user.get("auth_type") == "token":
            cost = scrape_credit_cost(is_all=(state == "all"), top25_on=top25_on)
            ok_c, msg_c, left = deduct_token_credits(user["token"], cost)
            if not ok_c:
                st.session_state.err = msg_c
                st.error(msg_c)
                st.stop()
            st.session_state.auth_user = refresh_token_session(user["token"]) or {
                **user, "credits": left
            }
            user = st.session_state.auth_user
        ...
        try:
            ...
            if save_disk:
                if is_all and top25_on:
                    perm = os.path.join(
                        OUTPUT_DIR, f"top25-{sport}-{mdy_to_iso(mdy)}.txt"
                    )
                    write_blocks(perm, blocks, overwrite=overwrite)
                elif is_all:
                    write_blocks(
                        all_states_file(sport, mdy), blocks, overwrite=overwrite
                    )
                else:
                    write_blocks(
                        state_file(state, sport), blocks, overwrite=overwrite
                    )
